...
func (s *Server) Process(ctx context.Context, network net.Network, conn stat.Connection, dispatcher routing.Dispatcher) error {
	...
	if v, ok := iConn.(User); ok {
		inbound.User = v.User()
		if inbound.User != nil {
			// QUIC authentication is cached for the connection. Recheck each
			// stream so removing or rotating a managed account takes effect.
			current := s.validator.GetByEmail(inbound.User.Email)
			if current == nil || !current.Account.Equals(inbound.User.Account) {
				return errors.New("hysteria account revoked")
			}
