...
func CopyRawConnIfExist(ctx context.Context, readerConn net.Conn, writerConn net.Conn, writer buf.Writer, timer *signal.ActivityTimer, inTimer *signal.ActivityTimer, download bool) error {
	direction := "upload"
	if download {
		direction = "download"
	}
	...
	for {
		...
		if splice {
			...
			var err error
			for {
				if release, allowed := aswired.TryUnboundedSplice(ctx); allowed {
					var n int64
					n, err = tc.ReadFrom(readerConn)
					aswired.ObserveSplice(ctx, direction, false, n)
					w += n
					release()
					break
				}
				var n int64
				n, err = tc.ReadFrom(io.LimitReader(readerConn, 64<<10))
				aswired.ObserveSplice(ctx, direction, true, n)
				w += n
				if n > 0 {
					timer.Update()
					if inTimer != nil {
					...
					if limitErr := aswired.TransferDirection(ctx, n, direction); limitErr != nil {
						err = limitErr
						break
					}
				}
