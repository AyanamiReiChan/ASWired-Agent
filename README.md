...
On initialization, a missing Xray configuration is created at `xray_config` (by default `data_dir/xray/config.json`) with private file permissions, no inbound listeners, direct/block outbounds and traffic statistics enabled. The embedded core can start and its configuration/hash can synchronize immediately, before any inbound is deployed. Existing files are never replaced during initialization, including invalid configurations that require repair.
...
Upgraded WebSocket peers negotiate binary frames and optional gzip inside the authenticated legacy handshake. In that mode, lightweight heartbeats run independently every 15 seconds, telemetry retains a 5-second sampling interval, and command results are sent immediately and retained until acknowledged. Static capabilities are sent on change. Only telemetry is compressed, before encryption, when it is large enough and compression saves bytes; credentials and task contents are not mixed into compression. Legacy controllers, HTTP Direct and Pull keep their original v1 exchange and intervals. All encrypted JSON avoids unnecessary HTML escaping. Configuration hashes let an upgraded controller skip unchanged full configuration downloads.

Peers that negotiate `telemetry_delta` send a full baseline on connection, then only changed fields and cumulative counters. Each update carries a generation and sequence acknowledged by the controller; an unacknowledged baseline is never used. Reconnection and core generation changes send a fresh snapshot. Sampling continues while waiting for an acknowledgement, and heartbeats remain independent.

Keep actual server addresses, personal domains, account and node identities, enrollment tokens, SSH records and recovery notes in ignored `.deployment/` storage or outside the repository. Commit only example configuration. Review staged files and scan Git history before pushing; ignore rules do not remove already tracked secrets.
